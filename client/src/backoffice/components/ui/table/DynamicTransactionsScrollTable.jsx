...
export default function DynamicTransactionsScrollTable({ 
    data = [], 
    columns, 
    onAction,
    selectedRow, 
    ...
    message,
    actions = [],
    rowSelect = false, // Enable row selection only when needed
    selectedRowValue,
    ...
    const headerCols = `p-2 pl-[10px] font-bold text-left cursor-pointer select-none`
    const actionHeaderCol = `p-2 text-right font-bold pr-[${actionColHeaderPaddingRight}]`
    const tableRow = `flex w-full h-[40px] border-b border-b-[#E5E5E5] text-left text-[1.2rem] pl-[0px] text-left`
    ...
    const headers = useMemo(() => {
        return columns.map(col => ({
            ...
            render: col.render // ✅ preserve the render function
        }))
    }, [columns])
    ...
    return (
        ...
        >
            ...
            >
                    ...
                    sortedData.map((row, index) => (
                        <div 
                            ...
                            style={{
                                ...
                                backgroundColor:
                                    rowSelect && selectedRowValue !== undefined && selectedRowValue === row.category
                                        ? "#BAE6FD"
                                        : "transparent"
                            }}
                        >
                            ...
                            {actions.length > 0 &&
                                <div className="relative">
                                    <div 
                                        className={actionRowColContainer} 
                                    >
                                        {actions.includes("view") && 
                                            <Eye onClick={() => onAction("view", row.transaction_id)} size={20} strokeWidth={2.2} style={{ cursor: "pointer"}} />
                                        }
                ...
                {/* NEED FOR PADDING AT BOTTOM SO ROWS SCROLL UP BELOW BOITTOM OF TABLE */}
                {sortedData.length > 15 && (
                    <>
                    <div key={"blank_row_1"} className={tableRow}></div>
                    <div key={"blank_row_2"} className={tableRow}></div>
                    <div key={"blank_row_3"} className={tableRow}></div>
                    </>
                )}
            </div>
